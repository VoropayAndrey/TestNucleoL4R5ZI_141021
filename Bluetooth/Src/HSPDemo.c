...
static DWord_t			  HDSServerSDPHandle;	   /* Variable used to hold the 	  */
 												   ...
 												   /* Servic Record of the Headset or */
   ...
   if(CommandParser(&TempCommand, Command) >= 0)
   {
      ...
      switch(Result)
      {
         ...
         case EXIT_CODE:
			 ...
			 /* go ahead an close any Remote Audio Gateway		*/
			 /* Server that we have open.						*/
			 if(HDSClientID)
				CloseRemoteServer(NULL);
			 
			 /* Close the Local Headset Server that we have 	*/
			 /* open.											*/
			 //CloseServer();
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   DisplayPrompt();
	...
	/* Audio Gateway Server on the Local Device.  This function opens the*/
	/* Headset or Audio Gateway Server on RFCOMM on Channel 1.	This	 */
...
static int OpenServer(ParameterList_t *TempParam)
{
	...
	if(BluetoothStackID)
	{
	   ...
	   if(UI_Mode == UI_MODE_IS_SERVER)
	   {
		  ...
		  /* If the Open was successful, then note the Headset Server ID.*/
		  if(ret_val > 0)
		  {
			 ...
			 else
			 {
				...
				/* correctly.											 */
				Display(("Headset Server Opened (Channel = %u).\r\n", LOCAL_SERVER_CHANNEL_ID));
	   ...
	   else
	   {
		  ...
		  if(ret_val > 0)
		  {
			 ...
			 /* If there was an error creating the Audio Gateway Server's*/
...
static int CloseServer(ParameterList_t *TempParam)
{
	...
	if(BluetoothStackID)
	{
	   ...
	   if(HDSServerID)
	   {
		  ...
		  if(HDSServerSDPHandle)
		  {
			 ...
			 /* Flag that there is no longer an SDP Headset/Audio Gateway*/
			 /* Server Record.											 */
		  ...
		  /* Flag that there is no Headset/Audio Gateway Server currently*/
		  /* open.														 */
		  HDSServerID = 0;
	...
	return(ret_val);
	 ...
	 /* with a Remote Headset or Audio Gateway Server.	This function	  */
	...
	if(BluetoothStackID)
	{
		...
		if(HDSClientID)
		{
		   ...
		   if(UI_Mode == UI_MODE_IS_SERVER)
		   {
			...
			{
				Display(("Audtio Gateway Client closed successfully.\r\n"));
		  		/* Simply close the Audio Gateway Client. 				  */
				HDSET_Close_Port(BluetoothStackID, HDSClientID);
	  		 	/* Flag that there is no longer a Headset Client connected. */
				HDSClientID			 = 0;
		...
		{
			/* Display an error to the user informing them that no Headset */
			/* or Audio Gateway Client is open.							  */
			if(UI_Mode == UI_MODE_IS_SERVER)
...
static int PressButton(ParameterList_t *TempParam)
{
   ...
   /* (Volume) Change Command to the Remote Connection.  This    */
   /* function returns zero if successful or a negative return error      */
   /*code if an error occurs.                                                           */
static int ChangeSpeakerGain(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(((ServerConnected)?HDSServerID:HDSClientID))
      {
        ...
        if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam) && (TempParam->Params[0].intParam >= HDSET_SPEAKER_GAIN_MINIMUM) && (TempParam->Params[0].intParam <= HDSET_SPEAKER_GAIN_MAXIMUM))
        {
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            Display(("Usage: ChangeVolume [Volume Value (0-15)].\r\n"));
	...
	if(BluetoothStackID)
	{
		...
		if(UI_Mode == UI_MODE_IS_CLIENT)
		{
			...
			if(((ServerConnected)?HDSServerID:HDSClientID))
			{
				...
				{
				   /* Ring Indication was issued unsuccessfully so flag an	*/
				   /* error to the user.									*/
...
static int ChangeMicrophoneGain(ParameterList_t *TempParam)
{
	...
	if(BluetoothStackID)
	{
		...
		{
			/* Next make sure that valid parameters were specified.		*/
			if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam) && (TempParam->Params[0].intParam >= HDSET_MICROPHONE_GAIN_MINIMUM) && (TempParam->Params[0].intParam <= HDSET_MICROPHONE_GAIN_MAXIMUM))
			{
				...
				{
				   /* Microphone Gain was issued unsuccessfully so flag an	*/
				   /* error to the user.									*/
...
static void BTPSAPI GAP_Event_Callback(unsigned int BluetoothStackID, GAP_Event_Data_t *GAP_Event_Data, unsigned long CallbackParameter)
{
   ...
   else
   {
	...
	/* processing in this function should be as efficient as possible    */
	/* (this argument holds anyway because another Headset Profile Event */
	/* will not be processed while this function call is outstanding).   */
	/* * NOTE * This function MUST NOT Block and wait for events that    */
	/*		  can only be satisfied by Receiving other Headset Events. */
	/*		  A Deadlock WILL occur because NO GAP Event Callbacks will*/
	/*		  be issued while this function is currently outstanding.  */
static void BTPSAPI HDSET_Event_Callback(unsigned int BluetoothStackID, HDSET_Event_Data_t *HDSET_Event_Data, unsigned long CallbackParameter)
{
	char		   BoardStr[13];
	...
	if((BluetoothStackID) && (HDSET_Event_Data))
	{
		...
		switch(HDSET_Event_Data->Event_Data_Type)
		{
			...
			case etHDSET_Audio_Disconnection_Indication:
			   /* An Audio Disconnect Indication was received, display the */
			   /* relevant information. 								   */
