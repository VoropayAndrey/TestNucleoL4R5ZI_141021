...
#include "AUDIO.h"          /* Audio Abstraction Layer Header.           */
...
static void Change_connection_priority(int high_normal, Word_t  ConnHandle);
...
static void BTPSAPI GAP_Event_Callback(unsigned int BluetoothStackID, GAP_Event_Data_t *GAP_Event_Data, unsigned long CallbackParameter);
   ...
   /* The following function is responsible for initalizing the         */
...
static int Initialize_Sink(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!AUDInitialized)
      {
         ...
         RemoteControlRoleInfo.ServiceName                  = "STM32 A3DP Sink";
         ...
         /* Reset the DAC for eliminating noises after the STM3240G-EVAL board resets */
         AUDIO_Reset_CODEC();
         ...
         if(!ret_val)
         {
            ...
            /* We need to change the bitpool parameters to make sure    */
            /* that the range is appropriate for the A3DP processor.    */
            AUD_Change_Media_Codec_Parameters(BluetoothStackID, astSNK, 
            						BTPS_CONFIGURATION_AUD_SBC_MINIMUM_BIT_POOL_VALUE, 0x2A);
         }
         else
...
{  
   ...
   */
   ...
   else if((0 == ConnHandle) && (0 != saved_ConnHandle))
   {
 	  /* If we got connection handle, from last called function, retrive it! */
  ...
  if(HIGH_AUDIO_CONNECTION_PRIORITY == high_normal)
  {
	 ...
	 if(ConnHandle)
	 {
	     ...
	     HCI_Flow_Specification(BluetoothStackID, ConnHandle, Flags, Flow_Direction, Service_Type, 
...
static int ReconfigureA3DPStream(AUD_Stream_Format_t *Format)
{
   ...
   Display((" Initialize audio with Sample Freq: %lu\r\n", Format->SampleFrequency));
   initializeAudio(BluetoothStackID, Format->SampleFrequency);
      ...
      if(ret_val == 0)
      {
         ...
         if(ret_val == 0)
         {
            ...
            if(ret_val == 0)
            {
			   ...
			   /* Change the ACL connection priority to high, for better audio quality with Bluetooth scans */
			   Change_connection_priority(HIGH_AUDIO_CONNECTION_PRIORITY, ConnHandle);
			   /* Configure the audio parameters also for the DAC, CC256x PCM interface and the STM32f I2S */
...
static int CloseA3DPStream(void)
{
   ...
   if(A3DPOpened)
   {
      ...
      /* Change the ACL connection priority back to normal, for better Bluetooth scans */
	  Change_connection_priority(NORMAL_AUDIO_CONNECTION_PRIORITY, 0);

      ret_val = VS_A3DP_Sink_Close_Stream(BluetoothStackID);
      Display(("A3DP Close: %d\r\n", ret_val));
      uninitializeAUDIO();

      /* Set to closed even if an error occurs.                         */
      A3DPOpened  = FALSE;
...
static int CloseA3DPEndpoint(ParameterList_t *TempParam)
{
   ...
   /* Verify that AUD was initialized succesfully.                      */
   if(AUDInitialized)
   {
...
/* The following function is responsible for checking changing the   */
...
/* 
   ...
   mode, starts the STM32f I2S as master and starts the CC43L22 DAC.
   Input : The desire sampling frequency.
*/
static int PcmLoopback(ParameterList_t *TempParam)
{
   int ret_val = 0;
   ...
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
	 ...
	 {
		Display((" No valid PCM sampling frequency (48000 or 44100) been given !!! \r\n"));
	 }
   }
   else
   {
