...
#include "SS1BTHRS.h"            /* Main SS1 HRS Service Header.              */
...
typedef struct _tagDeviceInfo_t
{
   ...
   HRS_Client_Information_t ClientInfo;
   HRS_Server_Information_t ServerInfo;
...
static unsigned int        HRSInstanceID;           /* The following holds the HRP     */
                                                    /* Instance ID that is returned    */
                                                    /* from HRS_Initialize_Service().  */
...
static Word_t              CurrentEnergyExpended;   /* Variable which holds the current*/
...
static int RegisterHRS(ParameterList_t *TempParam);
static int UnregisterHRS(ParameterList_t *TempParam);
static int DiscoverHRS(ParameterList_t *TempParam);
static int ConfigureRemoteHRS(ParameterList_t *TempParam);
static int NotifyHeartRate(ParameterList_t *TempParam);
static int GetBodySensorLocation(ParameterList_t *TempParam);
static int SetBodySensorLocation(ParameterList_t *TempParam);
static int ResetEnergyExpended(ParameterList_t *TempParam);

   /* Heart Rate Profile Helper Functions                               */
static void HRSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
static HRS_Heart_Rate_Measurement_Data_t *DecodeDisplayHeartRate(unsigned int ValueLength, Byte_t *Value, Boolean_t PrintData);
...
static void BTPSAPI HRS_EventCallback(unsigned int BluetoothStackID, HRS_Event_Data_t *HRS_Event_Data, unsigned long CallbackParameter);
...
static void GAPSPopulateHandles(GAPS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
   ...
   /* the information discovered from a GATT Service Discovery Discovery*/
   /* operation.                                                        */
static void HRSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
...
static int GetRemoteAppearance(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for registering a HRP       */
...
static int RegisterHRS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!HRSInstanceID)
      {
         /* Register the HRP Service with GATT.                         */
         ...
         if((ret_val > 0) && (HRSInstanceID > 0))
         {
            ...
            /* Initialize internal HRS variables                        */
   ...
   /* The following function is responsible for unregistering a HRP     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterHRS(ParameterList_t *TempParam)
{
   ...
   if(HRSInstanceID)
   {
      ...
      /* Unregister the HRP Service with GATT.                          */
   ...
   /* The following function is responsible for performing a HRP        */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverHRS(ParameterList_t *TempParam)
{
   ...
   if(!HRSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the HRP Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
   ...
   /* The following function is responsible for configure a HRP Service */
...
static int ConfigureRemoteHRS(ParameterList_t *TempParam)
{
         ...
         if(ConnectionID)
         {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  /* Determine if Heart Rate Measurement Notification is*/
                  /* supported (mandatory).                             */
   ...
   /* The following function is responsible for performing a Heart Rate */
...
static int NotifyHeartRate(ParameterList_t *TempParam)
{
   HRS_Heart_Rate_Measurement_Data_t *HeartRatePtr;
   DeviceInfo_t                      *DeviceInfo;
   Word_t                             Index;
   int                                ret_val = FUNCTION_ERROR;
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((TempParam) && (TempParam->NumberofParameters >= 5))
            {
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  {
                     /* Allocate a buffer for the requested number of   */
                     /* intervals.                                      */
                     HeartRatePtr = BTPS_AllocateMemory(HRS_HEART_RATE_MEASUREMENT_DATA_SIZE(TempParam->Params[4].intParam));

                     /* Only continue if we have a pointer to a         */
                     /* measurement structure to send.                  */
                     if(HeartRatePtr)
                     {
                        /* Begin formatting the Heart Rate Measurement. */
                        ...
                        /* Determine if we are going to send the Energy */
                        /* Expended field.                              */
                        if(TempParam->Params[3].intParam)
                        {
                           /* We are going to send the Energy Expended  */
                           /* Field.                                    */
                        ...
                        /* If there are any RR Intervals in the         */
                        /* measurement then add as many as specified by */
                        /* the user.                                    */
                        if(TempParam->Params[4].intParam)
                        {
                           /* Flag that RR Intervals are present.       */
                           ...
                           /* Loop and add all of the RR Intervals.     */
                           Index = 0;
                        ...
                        /* Go ahead and send the Heart Rate Measurement.*/
                        ...
                        /* Free the Heart Rate Measurement memory.      */
   ...
   /* The following function is responsible for reading the Body Sensor */
   /* Location characteristic.  It can be executed by a server or a     */
   /* client with an open connection to a remote server.  This function */
...
static int GetBodySensorLocation(ParameterList_t *TempParam)
{
   ...
   /* First check for a registered HRP Server                           */
   if(HRSInstanceID)
   {
   ...
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Verify that the client has received a valid BSL Attribute*/
            /* Handle.                                                  */
   ...
   /* The following function is responsible for writing the Body Sensor */
   /* Location characteristic.  It can be executed only by a server.    */
...
static int SetBodySensorLocation(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for writing the Reset Energy*/
   /* Expended command to a remote server Control Point.  It can be     */
   /* executed only by a client.  This function will return zero on     */
   /* successful execution and a negative value on errors.              */
static int ResetEnergyExpended(ParameterList_t *TempParam)
{
   DeviceInfo_t *DeviceInfo;
   Byte_t        CommandBuffer[HRS_HEART_RATE_CONTROL_POINT_VALUE_LENGTH];
   int           ret_val = FUNCTION_ERROR;
...
static void BTPSAPI GAP_LE_Event_Callback(unsigned int BluetoothStackID, GAP_LE_Event_Data_t *GAP_LE_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* The following is a HRP Server Event Callback.  This function will */
   /* be called whenever an HRP Server Profile Event occurs that is     */
   /* associated with the specified Bluetooth Stack ID.  This function  */
   /* passes to the caller the Bluetooth Stack ID, the HRS Event Data   */
   /* that occurred and the HRS Profile Event Callback Parameter that   */
   /* was specified when this Callback was installed.  The caller is    */
   /* free to use the contents of the HRS Profile Event Data ONLY in the*/
   ...
   /* another HRS Profile Event will not be processed while this        */
...
static void BTPSAPI HRS_EventCallback(unsigned int BluetoothStackID, HRS_Event_Data_t *HRS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (HRS_Event_Data))
   {
      ...
      {
         case etHRS_Server_Read_Client_Configuration_Request:
            ...
            break;
         case etHRS_Server_Client_Configuration_Update:
            ...
            break;
         case etHRS_Server_Heart_Rate_Control_Point_Command:
            ...
            if(HRS_Event_Data->Event_Data.HRS_Heart_Rate_Control_Command_Data)
            {
               ...
               switch(HRS_Event_Data->Event_Data.HRS_Heart_Rate_Control_Command_Data->Command)
               {
                  case ccResetEnergyExpended:
                     /* Reset the internal energy expended value        */
                     CurrentEnergyExpended = 0;
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValueLength != 0)
               {
                  if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != NULL))
                  {
                     ...
                     {
                        /* Could not find a descriptor to match the read*/
                        /* response, so display raw data                */
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Check the service discovery type for HRS           */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdHRS)
                  {
                     /* Attempt to populate the handles for the HRP     */
                     /* Service.                                        */
