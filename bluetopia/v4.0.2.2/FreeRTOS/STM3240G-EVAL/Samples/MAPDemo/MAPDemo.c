...
//static char                InputIndex;              /* Index into the Input Buffer     */
                                                    /* which holds the user input to   */
                                                    /* be parsed.                      */
      ...
      if(CurrentOperation == coNone)
      {
         ...
         if(!Temp)
         {
            ...
            if((NumberMessages = QueryNumberMessageEntries(GetCurrentMessageFolder())) >= 0)
            {
               ...
               /* structure.                                            */
               /* TODO By customer on embedded plaforms.                */
      ...
      if(CurrentOperation == coNone)
      {
         ...
         if(!Temp)
         {
            ...
            if(GenerateUniqueMessageHandle(sizeof(MessageHandle), MessageHandle))
            {
               ...
               /* delimeter character at the end of the Current Receive */
...
static int SetBaudRate(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0))
      {
         ...
         {
            if(VS_Update_UART_Baud_Rate(BluetoothStackID, (DWord_t)(TempParam->Params[0].intParam)))
            {
               Display(("VS_Update_UART_Baud_Rate Success.\r\n"));
               ret_val = 0;
            }
            else
            {
               Display(("VS_Update_UART_Baud_Rate Failure.\r\n"));
               ret_val = FUNCTION_ERROR;
            }
         }
         else
         ...
         case etAuthentication:
            ...
            switch(GAP_Event_Data->Event_Data.GAP_Authentication_Event_Data->GAP_Authentication_Event_Type)
            {
               ...
               case atLinkKeyCreation:
                  ...
                  if(Index != (-1))
                  {
                     /* * NOTE * We do not store the Link Key if the other */
                     /*          side specified no bonding (for SSP).  If  */
                     /*          this was legacy pairing then we need to   */
                     /*          store the Link Key always.                */
