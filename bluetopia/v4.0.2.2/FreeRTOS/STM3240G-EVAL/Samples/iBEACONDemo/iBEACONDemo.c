...
static int SetIbeaconUUID(ParameterList_t *TempParam)
{
   ...
   char         *temp_ptr    = NULL;
    ...
    {        
        ...
        {
            /* Puts '-' in the end of the string in order to exit   */
            /* the inner loop in the end of the string              */
...
static int AdvertizeIbeacon(ParameterList_t *TempParam)
{
   ...
   GAP_LE_Connectability_Parameters_t  ConnectabilityParameters;
    ...
    if(BluetoothStackID)
    {
        ...
        /* set the index to the IBEACON UUID location at the Advertising Data struct*/
        /*and Set the IBEACON UUID to the Advertising Data */                  
...
int InitializeApplication(HCI_DriverInformation_t *HCI_DriverInformation, BTPS_Initialization_t *BTPS_Initialization)
{
   ...
   if((HCI_DriverInformation) && (BTPS_Initialization))
   {
      ...
      {        
         /* Advertise Deafult values at Begining*/
         if ((ret_val = AdvertizeIbeacon(NULL)) == 0)
         {
             Display(("************************************************************\r\n"));
             Display((" Application is already Advertising deafult values,         \r\n"));
             ...
             Display(("************************************************************\r\n"));
             Display(("The deafult values are:                                     \r\n"));
