...
#define APP_DEMO_NAME    "STM32-TI-BT-SRC-%02X%02X%02X"  /* Defines the format*/
                                                         /* device name of the*/
                                                         /* prefix.           */
...
#define INQUIRY_REASON_AUTO_CONNECT                (1)
   ...
   /* found, then the inquiry process is automatically cancelled.       */
...
static int Initialize_Source(void)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!AUDInitialized)
      {
         ...
         InitializationInfoSRC.EndpointSDPDescription       = "MSP430 A3DP Source";
         ...
         RemoteControlRoleInfo.ServiceName                  = "STM32 A3DP Source";
...
static int SetBaudRate(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam))
      {
         ...
         Display(("VS_Update_UART_Baud_Rate(%lu): ", TempParam->Params[0].intParam));
...
static int OpenRemoteEndpoint(ParameterList_t *TempParam)
{
   ...
   if((BluetoothStackID) && (TempParam) && (TempParam->NumberofParameters > 0))
   {
      /* Make sure the first parameter lies within our valid range.     */
      if((TempParam->Params[0].intParam) && (TempParam->Params[0].intParam <= NumberofValidResponses))
      {
