...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  PASPDemo - Embedded Bluetooth Phone Alert Status Profile using GATT (LE)  */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
#include "PASPDemo.h"            /* Application Header.                       */
...
#include "BTVSAPI.h"             /* Vendor Specific Prototypes/Constants.     */
...
#define LE_APP_DEMO_NAME                        "PASPDemo"
         ...
         for(Index1 = 0; Index1 < ServiceDiscoveryData->NumberOfCharacteristics; Index1++, CurrentCharacteristic++)
         {
            ...
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               ...
               if(!PASS_COMPARE_PASS_ALERT_STATUS_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  if(!PASS_COMPARE_PASS_RINGER_SETTING_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
                  {
                     ...
                     else
                     {
                        ...
                        if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE_WITHOUT_RESPONSE))
                           Display(("Warning - Mandatory write without response property of Ringer Control Point characteristic not supported!\r\n"));
                  ...
                  else
                  {
                     ...
                     if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                        Display(("Warning - Mandatory read property of Ringer Setting characteristic not supported!\r\n"));
               ...
               else
               {
                  ...
                  /* Verify that notify is supported.                   */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_NOTIFY))
                     Display(("Warning - Mandatory notify property of Alert Status characteristic not supported!\r\n"));
               }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   Display(("* Command Options PASP:    RegisterPASS, UnregisterPASS,         *\r\n"));
   Display(("*                          DiscoverPASS, ConfigureRemotePASS,    *\r\n"));
   Display(("*                          NotifyAlertStatus,                    *\r\n"));
   Display(("*                          NotifyRingerSetting,                  *\r\n"));
   Display(("*                          GetAlertStatus, SetAlertStatus,       *\r\n"));
   Display(("*                          GetRingerSetting, SetRingerSetting,   *\r\n"));
   Display(("*                          SetSilentMode, CancelSilentMode,      *\r\n"));
   Display(("*                          MuteOnce                              *\r\n"));
   Display(("******************************************************************\r\n"));
   return(0);
}
...
static int RegisterPASS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!PASSInstanceID)
      {
         ...
         ret_val = PASS_Initialize_Service(BluetoothStackID, PASS_EventCallback, NULL, &PASSInstanceID);
         if((ret_val > 0) && (PASSInstanceID > 0))
         {
            /* Display success message.                                 */
            Display(("Successfully registered PASP Service.\r\n"));
      ...
      {
         Display(("PASP Service already registered.\r\n"));
...
static int UnregisterPASS(ParameterList_t *TempParam)
{
   ...
   if(PASSInstanceID)
   {
      ...
      {
         /* Display success message.                                    */
         Display(("Successfully unregistered PASP Service.\r\n"));
      ...
      Display(("PASP Service not registered.\r\n"));
...
static int DiscoverPASS(ParameterList_t *TempParam)
{
   ...
   if(!PASSInstanceID)
   {
      ...
      Display(("Cannot discover PASP Services when registered as a service.\r\n"));
...
static int ConfigureRemotePASS(ParameterList_t *TempParam)
{
   ...
   if(TempParam && (TempParam->NumberofParameters > 1))
   {
      ...
      if(!PASSInstanceID)
      {
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Alert_Status_Client_Configuration)
                  {
                     ...
                     Display(("   Error - Alert Status CC not found on this device.\r\n"));
                  ...
                  if(ret_val > 0)
                  {
                     ...
                     {
                        Display(("   Error - Ringer Setting CC not found on this device.\r\n"));
         ...
         Display(("Cannot configure remote PASP Services when registered as a service.\r\n"));
   }
   else
   {
      Display(("Usage: ConfigureRemotePASS [Alert Status Notify (0 = disable, 1 = enable)]\r\n"));
      Display(("                           [Ringer Setting Notify (0 = disable, 1 = enable)]\r\n"));
   }
   ...
   if(BluetoothStackID)
   {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->ServerInfo.Alert_Status_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  ...
                  {
                     Display(("Alert Status Notification success.\r\n"));
                  }
                  else
               ...
               {
                  Display(("Client has not registered for Alert Status notifications.\r\n"));
      ...
      else
      {
         if(ConnectionID)
            ...
            Display(("Error - PASP server not registered\r\n"));
      }
   }
   else
   {
         ...
         if((ConnectionID) && (!COMPARE_NULL_BD_ADDR(ConnectionBD_ADDR)))
         {
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               if(DeviceInfo->ServerInfo.Ringer_Setting_Client_Configuration & GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE)
               {
                  ...
                  {
                     Display(("Ringer Setting Notification success.\r\n"));
                  }
                  else
                     DisplayFunctionError("PASS_Send_Notification", ret_val);
               }
               ...
               {
                  Display(("Client has not registered for Ringer Setting notifications.\r\n"));
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Alert_Status != 0)
            {
               ...
               {
                  Display(("Get Alert Status Request sent, Transaction ID = %u", ret_val));
               ...
               Display(("Error - Alert Status not supported on remote service.\r\n"));
         }
         else
            Display(("No Device Info.\r\n"));
      }
      else
         Display(("Either a PASP server must be registered or a PASP client must be connected.\r\n"));
   }
   ...
   if((TempParam) && (TempParam->NumberofParameters > 2))
   {
      ...
      if(PASSInstanceID)
      {
         ...
         if(((ret_val = PASS_Query_Ringer_Setting(BluetoothStackID, PASSInstanceID, &RingerSetting)) == 0) && ((RingerSetting != rsSilent) || (TempParam->Params[0].intParam == 0)))
         {
            ...
            {
               Display(("Alert Status successfully set.\r\n"));
            }
            else
               DisplayFunctionError("PASS_Set_Alert_Status", ret_val);
         }
         else
            Display(("Error - Cannot activate ringer when the ringer is set to silent."));
      }
      else
      {
         if(ConnectionID)
            Display(("Cannot write to Alert Status as a client.\r\n"));
         else
            Display(("PASP server not registered\r\n"));
      }
   }
   else
   {
      Display(("Usage: SetAlertStatus [Ringer  (0 = inactive, 1 = active)]\r\n"));
      Display(("                      [Vibrate (0 = inactive, 1 = active)]\r\n"));
      Display(("                      [Display (0 = inactive, 1 = active)]\r\n"));
   }
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Ringer_Setting != 0)
            {
               ...
               {
                  Display(("Get Ringer Setting Request sent, Transaction ID = %u", ret_val));
               ...
               Display(("Error - Ringer Setting not supported on remote service.\r\n"));
         }
         else
            Display(("No Device Info.\r\n"));
      }
...
static int SetRingerSetting(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters > 0))
   {
      if(PASSInstanceID)
      {
         ...
         {
            Display(("Ringer Setting successfully set.\r\n"));
         }
         else
      ...
      {
         if(ConnectionID)
            Display(("Cannot write to Ringer Setting as a client.\r\n"));
         else
            Display(("PASP server not registered\r\n"));
      }
   }
...
static int SetSilentMode(ParameterList_t *TempParam)
{
   ...
   if(!PASSInstanceID)
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Ringer_Control_Point != 0)
            {
               ...
               if((ret_val = PASS_Format_Ringer_Control_Command(rcSilent, PASS_RINGER_CONTROL_POINT_VALUE_LENGTH, CommandBuffer)) > 0)
               {
                  ...
                  {
                     Display(("Set Silent Mode command request sent.\r\n"));
               ...
               Display(("Error - Ringer Control Point not supported on remote service!\r\n"));
         }
         else
      ...
      Display(("Cannot Set Silent Mode as a service.\r\n"));
...
static int CancelSilentMode(ParameterList_t *TempParam)
{
   ...
   if(!PASSInstanceID)
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Ringer_Control_Point != 0)
            {
               ...
               if((ret_val = PASS_Format_Ringer_Control_Command(rcCancelSilent, PASS_RINGER_CONTROL_POINT_VALUE_LENGTH, CommandBuffer)) > 0)
               {
                  ...
                  if((ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.Ringer_Control_Point, PASS_RINGER_CONTROL_POINT_VALUE_LENGTH, ((void *)CommandBuffer))) > 0)
                  {
                     Display(("Cancel Silent Mode command request sent.\r\n"));
      ...
      Display(("Cannot Cancel Silent Mode as a service.\r\n"));
...
static int MuteOnce(ParameterList_t *TempParam)
{
   ...
   if(!PASSInstanceID)
   {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(DeviceInfo->ClientInfo.Ringer_Control_Point != 0)
            {
               ...
               if((ret_val = PASS_Format_Ringer_Control_Command(rcMuteOnce, PASS_RINGER_CONTROL_POINT_VALUE_LENGTH, CommandBuffer)) > 0)
               {
                  ...
                  if((ret_val = GATT_Write_Without_Response_Request(BluetoothStackID, ConnectionID, DeviceInfo->ClientInfo.Ringer_Control_Point, PASS_RINGER_CONTROL_POINT_VALUE_LENGTH, ((void *)CommandBuffer))) > 0)
                  {
                     Display(("Mute Once command request sent.\r\n"));
      ...
      Display(("Cannot Mute Once as a service.\r\n"));
...
static void BTPSAPI PASS_EventCallback(unsigned int BluetoothStackID, PASS_Event_Data_t *PASS_Event_Data, unsigned long CallbackParameter)
{
   ...
   if((BluetoothStackID) && (PASS_Event_Data))
   {
      switch(PASS_Event_Data->Event_Data_Type)
      {
         case etPASS_Server_Read_Client_Configuration_Request:
            Display(("etPASS_Server_Read_Client_Configuration_Request with size %u.\r\n", PASS_Event_Data->Event_Data_Size));
            ...
            if(PASS_Event_Data->Event_Data.PASS_Read_Client_Configuration_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, PASS_Event_Data->Event_Data.PASS_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
               {
                  ...
                  if(PASS_Event_Data->Event_Data.PASS_Read_Client_Configuration_Data->InstanceID == PASSInstanceID)
                  {
                     ...
                     {
                        case rrAlertStatus:
                           Display(("   Config Type:      rrAlertStatus.\r\n"));
                        ...
                        case rrRingerSetting:
                           Display(("   Config Type:      rrRingerSetting.\r\n"));
         ...
         case etPASS_Server_Client_Configuration_Update:
            Display(("etPASS_Server_Client_Configuration_Update with size %u.\r\n", PASS_Event_Data->Event_Data_Size));
         ...
         case etPASS_Server_Ringer_Control_Command_Indication:
            Display(("etPASS_Server_Ringer_Control_Command_Indication with size %u.\r\n", PASS_Event_Data->Event_Data_Size));
            if(PASS_Event_Data->Event_Data.PASS_Ringer_Control_Command_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  switch(PASS_Event_Data->Event_Data.PASS_Ringer_Control_Command_Data->Command)
                  {
                     case rcSilent:
                        ...
                        if((Result = PASS_Set_Ringer_Setting(BluetoothStackID, PASSInstanceID, RingerSetting)) == 0)
                        {
                           if((Result = PASS_Query_Alert_Status(BluetoothStackID, PASSInstanceID, &AlertStatus)) == 0)
                           {
                              ...
                              if(Result == 0)
                                 Display(("Set Silent control command success.\r\n"));
                           }
                           else
                     ...
                     case rcMuteOnce:
                        ...
                        if((Result = PASS_Query_Alert_Status(BluetoothStackID, PASSInstanceID, &AlertStatus)) == 0)
                        {
                           if(AlertStatus.RingerStateActive)
                           {
                              ...
                              if((Result = PASS_Set_Alert_Status(BluetoothStackID, PASSInstanceID, AlertStatus)) == 0)
                                 Display(("Mute Once control command success.\r\n"));
                              else
                                 DisplayFunctionError("PASS_Set_Alert_Status", Result);
                           }
                           else
                              Display(("Mute Once control command received, but ringer is not active.\r\n"));
                        }
                        else
                           DisplayFunctionError("PASS_Query_Alert_Status", Result);
                        break;
                     ...
                     case rcCancelSilent:
                        Display(("   Command:          rcCancelSilent.\r\n\r\n"));
                        ...
                        if((Result = PASS_Set_Ringer_Setting(BluetoothStackID, PASSInstanceID, RingerSetting)) == 0)
                        {
                           /* Set the Ringer Alert State back to active.*/
                           if((Result = PASS_Query_Alert_Status(BluetoothStackID, PASSInstanceID, &AlertStatus)) == 0)
                           {
                              if(AlertStatus.RingerStateActive == FALSE)
                              {
                              ...
                              if(Result == 0)
                                 Display(("Cancel Silent control command success.\r\n"));
                           }
                           else
                              DisplayFunctionError("PASS_Query_Alert_Status", Result);
                        }
   ...
   else
   {
      ...
      Display(("PASS Callback Data: Event_Data = NULL.\r\n"));
   }
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice)) != NULL)
               {
                  if(LocalDeviceIsMaster)
                  {
                     ...
                     /* supported.                                      */
                     GATT_Exchange_MTU_Request(BluetoothStackID, ConnectionID, BTPS_CONFIGURATION_GATT_MAXIMUM_SUPPORTED_MTU_SIZE, GATT_ClientEventCallback_PASP, 0);
                  }
               }
            }
            else
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            ...
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  /* PASS.                                              */
                  if((GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data->Status == GATT_SERVICE_DISCOVERY_STATUS_SUCCESS) && (((Service_Discovery_Type_t)CallbackParameter) == sdPASS))
                  {
