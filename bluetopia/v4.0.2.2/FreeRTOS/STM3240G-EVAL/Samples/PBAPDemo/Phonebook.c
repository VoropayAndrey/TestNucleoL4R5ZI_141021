...
static CurrentDirectory_t CurrentDirectory;        /* Holds the current       */
                                                   /* directory that is       */
...
CurrentDirectory_t GetCurrentPhonebookDirectory(void)
{
...
char *GetCurrentPhonebookDirectoryString(void)
{
   char         *ret_val;
   unsigned int  Index;
   ...
   {
      if(DirectoryMapping[Index].Directory == CurrentDirectory)
      {
   ...
   /* error.                                                            */
int ChangePhonebookDirectory(PBAP_Set_Path_Option_t Option, char *Name)
{
   int          ret_val;
   unsigned int Index;
   unsigned int Index1;
   ...
   else
   {
      if(Option == spUp)
      {
         ...
         else
         {
            ...
            /* current directory we are currently at (to find the parent*/
            /* directory).                                              */
            for(Index=0,ret_val=-1;Index<sizeof(DirectoryStructure)/sizeof(DirectoryStructure_t);Index++)
            {
               if(DirectoryStructure[Index].CurrentDirectory == CurrentDirectory)
               {
                  /* Current directory found, note the parent directory,*/
                  /* flag success, and exit.                            */
                  CurrentDirectory = DirectoryStructure[Index].ParentDirectory;
      ...
      {
         /* Caller is requesting a sub-directory, make sure it's valid. */
         if((Name) && (BTPS_StringLength(Name)))
         {
            /* Try to see if the directory specified matches a directory*/
            /* name we have in our directory structure.                 */
            for(Index=0,ret_val=-1;Index<sizeof(DirectoryMapping)/sizeof(DirectoryNameMapping_t);Index++)
            {
               if((BTPS_StringLength(Name) == BTPS_StringLength(DirectoryMapping[Index].DirectoryName)) && (!BTPS_MemCompareI(Name, DirectoryMapping[Index].DirectoryName, BTPS_StringLength(Name))))
               {
                  ...
                  /* directory exists at the level we are currently at. */
                  for(Index1=0;Index1<sizeof(DirectoryStructure)/sizeof(DirectoryStructure_t);Index1++)
                  {
                     if(DirectoryStructure[Index1].CurrentDirectory == DirectoryMapping[Index].Directory)
                     {
                        if(DirectoryStructure[Index1].ParentDirectory == CurrentDirectory)
                        {
                           /* Match found, set the current directory,   */
                           /* flag succes, and exit.                    */
                           CurrentDirectory = DirectoryMapping[Index].Directory;
