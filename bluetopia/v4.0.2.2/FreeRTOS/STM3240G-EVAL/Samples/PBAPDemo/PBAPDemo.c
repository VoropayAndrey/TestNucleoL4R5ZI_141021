...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  PBAPDEMO - Simple embedded application using Phonebook Access Profile     */
/*             (PBAP).                                                        */
/*                                                                            */
/*  Author:  Ryan Byrne                                                      */
/*                                                                            */
...
/*   --------  -----------    ------------------------------------------------*/
/*   06/26/14  R. Byrne       Initial creation.                               */
/*   03/03/15  D. Horowitz    Adding Demo Application version.                */
/******************************************************************************/
#include "Main.h"                /* Application Interface Abstraction.        */
#include "PBAPDemo.h"            /* Application Header.                       */
#include "Phonebook.h"           /* Sample Phonebook Store Prototypes.        */
...
#include "SS1BTPBA.h"            /* PBAP Prototypes/Constants.                */
#include <stdio.h>               /* sscanf() prototype.                       */
...
typedef struct FW_Version_t
{
   ...
   /* Used to represent a structure to hold a BD_ADDR return from       */
   /* BD_ADDRToStr.                                                     */
...
static void ClearCommands(void)
{
   ...
   /* the specified data to console.                                    */
static void WriteReceivedData(unsigned int BufferLength, unsigned char *Buffer)
{
   char c;

   /* Save the last character so that the buffer is now null terminated.*/
   c                         = Buffer[BufferLength -  1];
   Buffer[BufferLength -  1] = '\0';

   Display(("\r\n%s%c\r\n", Buffer, c));

   /* Put the character back in case the buffer is used after this      */
   /* function.                                                         */
   c                         = Buffer[BufferLength -  1];
}
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   if(UI_Mode == UI_MODE_IS_CLIENT)
   {
      ...
      Display(("*                  OBEXDisconnect, PullPhonebook,                *\r\n"));
      Display(("*                  SetPhonebook, PullvCardList, PullvCARDEntry,  *\r\n"));
      Display(("*                  Abort, SetBaudRate, Help, Quit.               *\r\n"));
      Display(("******************************************************************\r\n"));
   }
   else
   {
      if(UI_Mode == UI_MODE_IS_SERVER)
      {
         ...
         Display(("*                  GetClassOfDevice, SetClassOfDevice,           *\r\n"));
         Display(("*                  GetRemoteName, Open, Close,                   *\r\n"));
         Display(("*                  Disconnect, SetBaudRate,                      *\r\n"));
         Display(("*                  Help, Quit.                                   *\r\n"));
         Display(("******************************************************************\r\n"));
      }
      else
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam))
      {
         ...
         if(!PBAPID)
         {
            ...
            if(Result > 0)
            {
               /* The Server was opened successfully.  The return value */
               /* of the call is the PBAP ID and is required for future */
               /* PBAP Profile calls.                                   */
               PBAPID = Result;

               /* Now that a Service Name has been created try and      */
               /* Register the SDP Record.                              */
               Result = PBAP_Register_Server_SDP_Record(BluetoothStackID, Result, "PBAP Server", &ServerSDPRecordHandle);

               if(!Result)
               {
                  ret_val = 0;

                  DisplayFunctionSuccess("PBAP_Register_Server_SDP_Record()");
               }
               else
               {
                  ...
                  DisplayFunctionError("PBAP_Register_Server_SDP_Record()", Result);
            ...
            else
            {
               ...
               DisplayFunctionError("PBAP_Open_Server_Port()", Result);
         ...
         {
            /* A Connection is already open, this program only    */
            /* supports one Server or Client at a time.           */
            Display(("PBAP Server already open.\r\n"));
   ...
   if(BluetoothStackID)
   {
      if(PBAPID)
      {
         ...
         if(!Result)
         {
            ...
            /* successfully closed.                                     */
            DisplayFunctionSuccess("PBAP_Close_Server_Port()");
         ...
         else
         {
            ...
            DisplayFunctionError("PBAP_Close_Server_Port()", Result);
      ...
      if(!PBAPID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters > 1) && (TempParam->Params[0].intParam) && (NumberofValidResponses) && (TempParam->Params[0].intParam<=NumberofValidResponses) && (!COMPARE_NULL_BD_ADDR(InquiryResultList[(TempParam->Params[0].intParam-1)])))
         {
            ...
            if(TempParam->Params[1].intParam)
            {
               ...
               Display(("Opening Remote PBAP Server (BD_ADDR = %s, Port = %04X)\r\n", BoardStr, TempParam->Params[1].intParam));
               ...
               if(Result > 0)
               {
                  ...
                  Display(("PBAP_Open_Remote_Server_Port: Function Successful (ID = %04X).\r\n", PBAPID));
               }
               else
               {
                  /* There was an error opening the Client.             */
                  Display(("PBAP_Open_Remote_Server_Port() Failure: %d.\r\n", Result));
            ...
            {
               DisplayUsage("Open [Inquiry Index] [Server Port]");
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if(!Result)
         {
            ...
            /* submitted.                                               */
            Display(("PBAP_Close_Connection: Function Successful.\r\n"));
            ...
            /*Server mode - PBAPID  is still valid since it described 	*/
			/* successfully call to PBAP_Open_Server_Port() function and*/
			/* it will be not valid only when calling 					*/
			/* PBAP_Close_Server_Port() .								*/
			
			/* Client mode -  PBAPID is not valid since it described 	*/
			/* successfully call to PBAP_Open_Remote_Server_Port()  	*/
			/* which open a connection	and when calling 				*/
			/* PBAP_Close_Connection() the connection is droped.		*/
            if (UI_Mode == UI_MODE_IS_CLIENT)
            {
                PBAPID = 0;
            }
         ...
         else
         {
            ...
            /* Connection.                                              */
            Display(("PBAP_Close_Connection() Failure: %d.\r\n", Result));
      ...
      else
      {
         ...
         /* open.                                                       */
         Display(("Invalid PBAP ID: PBAP Close Connection.\r\n"));
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if(!Result)
         {
            ...
            Display(("PBAP_Disconnect_Request: Function Successful.\r\n"));
         }
         else
         {
            /* There was an error submitting the function.              */
            Display(("PBAP_Disconnect_Request() Failure: %d.\r\n", Result));
      ...
      else
      {
         ...
         Display(("PBAP OBEX Disconnect Request: Invalid PBAPID.\r\n"));
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if(!Result)
         {
            ...
            Display(("PBAP_Abort_Request: Function Successful.\r\n"));
         }
         else
         {
            /* There was an error submitting the function.              */
            Display(("PBAP_Abort_Request() Failure: %d.\r\n", Result));
      ...
      {
         /* One or more of the parameters are invalid.                  */
         Display(("PBAP Abort Request: Invalid PBAPID.\r\n"));
...
static int PullPhonebook(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[2].strParam) && (BTPS_StringLength(TempParam->Params[2].strParam)))
         {
            ...
            {
               /* The function was submitted successfully.              */
               DisplayFunctionSuccess("PBAP_Pull_Phonebook_Request()");
            ...
            {
               /* There was an error submitting the function.           */
               DisplayFunctionError("PBAP_Pull_Phonebook_Request()", Result);
         ...
         {
            DisplayUsage("PullPhonebook [Max List Count] [List Start Offset] [Phonebook Path]");
      ...
      {
         /* One or more of the parameters are invalid.                  */
         Display(("Pull Phonebook Request: Invalid PBAPID.\r\n"));
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= (int)spRoot) && (TempParam->Params[0].intParam <= (int)spUp))
         {
            ...
            /* was specified.                                           */
            if((TempParam->Params[0].intParam != (int)spDown) || (((TempParam->Params[0].intParam == (int)spDown) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[1].strParam) && (BTPS_StringLength(TempParam->Params[1].strParam)))))
            {
               ...
               {
                  /* The function was submitted successfully.           */
                  DisplayFunctionSuccess("PBAP_Set_Phonebook_Request()");
               ...
               {
                  /* There was an error submitting the function.        */
                  DisplayFunctionError("PBAP_Set_Phonebook_Request()", Result);
            ...
            {
               DisplayUsage("SetPhonebook [Path Option (0 = Root, 1 = Down, 2 = Up)] [Path (only required for Down option)]");
      ...
      {
         /* One or more of the parameters are invalid.                  */
         Display(("Set Phonebook Request: Invalid PBAPID.\r\n"));
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if((TempParam) && (TempParam->NumberofParameters >= 2))
         {
            ...
            {
               /* The function was submitted successfully.              */
               DisplayFunctionSuccess("PBAP_Pull_vCard_Listing_Request()");
            ...
            {
               /* There was an error submitting the function.           */
               DisplayFunctionError("PBAP_Pull_vCard_Listing_Request()", Result);
         ...
         {
            DisplayUsage("PullvCardList [Max List Count] [List Start Offset] [Phonebook Path] ");
      ...
      {
         /* One or more of the parameters are invalid.                  */
         Display(("Pull vCard Listing Request: Invalid PBAPID.\r\n"));
...
static int PullvCardEntry(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         /* The PBAP ID appears to be is a semi-valid value.  Next,     */
         /* check to see if required parameters were specified.         */
         if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].strParam) && (BTPS_StringLength(TempParam->Params[0].strParam)))
         {
            ...
            {
               /* The function was submitted successfully.              */
               DisplayFunctionSuccess("PBAP_Pull_vCard_Entry_Request()");
            ...
            {
               /* There was an error submitting the function.           */
               DisplayFunctionError("PBAP_Pull_vCard_Entry_Request()", Result);
         ...
         {
            DisplayUsage("PullvCardEntry [vCard Name]");
      ...
      {
         /* One or more of the parameters are invalid.                  */
         Display(("Pull vCard Entry Request: Invalid PBAPID.\r\n"));
   ...
   if((BluetoothStackID) && (PBAPID) && (Connected) && (PBAP_Pull_Phonebook_Indication_Data))
   {
      ...
      if(CurrentOperation == coNone)
      {
         ...
         if((PBAP_Pull_Phonebook_Indication_Data->ObjectName) && (BTPS_StringLength(PBAP_Pull_Phonebook_Indication_Data->ObjectName)))
            Display(("Object Name: %s", PBAP_Pull_Phonebook_Indication_Data->ObjectName));
         else
            Display(("Invalid Object Name specified."));
         ...
         if(Directory != cdInvalid)
         {
            ...
            {
               Display(("Request for Number of Entries: %d\r\n", NumbervCards));
         ...
         {
            Display(("Invalid/Unsupported Phonebook specified.\r\n"));
      ...
      {
         Display(("Continuation for PBAP Pull Phonebook\r\n"));
      ...
      if(Result >= 0)
         Display(("PBAP_Pull_Phonebook_Response() Successful.\r\n"));
      else
         Display(("PBAP_Pull_Phonebook_Response() Failure %d.\r\n", Result));
   }
   else
...
static void ProcessSetPhonebook(PBAP_Set_Phonebook_Indication_Data_t *PBAP_Set_Phonebook_Indication_Data)
{
   ...
   if((BluetoothStackID) && (PBAPID) && (Connected) && (PBAP_Set_Phonebook_Indication_Data))
   {
      ...
      if(Result >= 0)
         Display(("PBAP_Set_Phonebook_Response() Successful.\r\n"));
      else
         Display(("PBAP_Set_Phonebook_Response() Failure %d.\r\n", Result));
   }
   else
      Display(("Invalid parameter or No longer connected.\r\n"));
}
...
static void ProcessPullvCardListing(PBAP_Pull_vCard_Listing_Indication_Data_t *PBAP_Pull_vCard_Listing_Indication_Data)
{
   ...
   if((BluetoothStackID) && (PBAPID) && (Connected) && (PBAP_Pull_vCard_Listing_Indication_Data))
   {
      ...
      if(CurrentOperation == coNone)
      {
         ...
         {
            Display(("Object Name: %s\r\n", PBAP_Pull_vCard_Listing_Indication_Data->ObjectName));
            ...
            else
            {
               ...
               /* error.                                                */
               Display(("Unable to pull vCard Listing: Invalid directory specified.\r\n"));
         ...
         if(!Temp)
         {
            ...
            if(vCardList)
            {
               ...
               else
               {
                  ...
                  if((CurrentBuffer = BTPS_AllocateMemory(CurrentBufferSize + 1)) != NULL)
                  {
                     ...
                     BTPS_SprintF((char *)CurrentBuffer, TELECOM_PHONEBOOK_LISTING_PREFIX);
            ...
            {
               Display(("Unable to pull vCard Listing: Invalid current directory.\r\n"));
      ...
      {
         Display(("Continuation for PBAP Pull vCard Listing\r\n"));
      ...
      if(Result >= 0)
         Display(("PBAP_Pull_vCard_Listing_Response() Successful.\r\n"));
      else
         Display(("PBAP_Pull_vCard_Listing_Response() Failure %d.\r\n", Result));
   }
   else
      Display(("Invalid parameter or No longer connected.\r\n"));
}
   ...
   if((BluetoothStackID) && (PBAPID) && (Connected) && (PBAP_Pull_vCard_Entry_Indication_Data))
   {
      ...
      if(CurrentOperation == coNone)
      {
         ...
         if((PBAP_Pull_vCard_Entry_Indication_Data->ObjectName) && (BTPS_StringLength(PBAP_Pull_vCard_Entry_Indication_Data->ObjectName)))
            Display(("Object Name: %s", PBAP_Pull_vCard_Entry_Indication_Data->ObjectName));
         else
            Display(("Invalid Object Name specified."));
         ...
         if(vCardList)
         {
            ...
            if(Index < NumbervCards)
            {
               ...
               /* * NOTE * We will allocate an extra byte to take care  */
               /*          of the NULL terminator.                      */
               if((CurrentBufferSize) && ((CurrentBuffer = BTPS_AllocateMemory(CurrentBufferSize + 1)) != NULL))
               {
                  /* Buffer allocated, go ahead and build the vCard     */
                  /* Entry buffer.                                      */
                  if(vCardList[Index])
               ...
               else
               {
                  if(CurrentBufferSize)
                     ...
                     Display(("Invalid vCard Entry specified.\r\n"));
         ...
         {
            Display(("Unable to pull vCard Entry: Invalid current directory.\r\n"));
      ...
      {
         Display(("Continuation for PBAP Pull vCard Entry\r\n"));
      ...
      if(Result >= 0)
         DisplayFunctionSuccess("PBAP_Pull_vCard_Entry_Response()");
      else
         DisplayFunctionError("PBAP_Pull_vCard_Entry_Response()", Result);
   }
   else
      Display(("Invalid parameter or No longer connected.\r\n"));
}
   ...
   if((BluetoothStackID) && (PBAPEventData))
   {
      ...
      switch(PBAPEventData->Event_Data_Type)
      {
         ...
         case etPBAP_Pull_vCard_Entry_Indication:
            Display(("Pull vCard Entry Indication\r\n"));
   ...
   if((BluetoothStackID) && (PBAPEventData))
   {
      ...
      switch(PBAPEventData->Event_Data_Type)
      {
         ...
         case etPBAP_Close_Port_Indication:
            ...
            if(PBAPEventData->Event_Data.PBAP_Close_Port_Indication_Data->PBAPID == (unsigned int)PBAPID)
            {
         ...
         case etPBAP_Pull_Phonebook_Confirmation:
            ...
            if(PBAPEventData->Event_Data.PBAP_Pull_Phonebook_Confirmation_Data->BufferSize)
            {
               ...
               for(Index = 0, TempCallbackBuffer[0] = '\0'; (Index < PBAPEventData->Event_Data.PBAP_Pull_Phonebook_Confirmation_Data->BufferSize) && (Index < (SIZEOF_TEMP_CALLBACK_BUFFER - 1));Index++)
                  BTPS_SprintF(&(TempCallbackBuffer[BTPS_StringLength(TempCallbackBuffer)]), "%c", PBAPEventData->Event_Data.PBAP_Pull_Phonebook_Confirmation_Data->Buffer[Index]);
            ...
            {
               Display(("Pull Phonebook not complete, Requesting more data.\r\n"));
         ...
         case etPBAP_Pull_vCard_Listing_Confirmation:
            ...
            if(PBAPEventData->Event_Data.PBAP_Pull_vCard_Listing_Confirmation_Data->BufferSize)
            {
               ...
               for(Index = 0, TempCallbackBuffer[0] = '\0'; (Index < PBAPEventData->Event_Data.PBAP_Pull_vCard_Listing_Confirmation_Data->BufferSize) && (Index < (sizeof(TempCallbackBuffer) - 1)); Index++)
                  BTPS_SprintF(&(TempCallbackBuffer[BTPS_StringLength(TempCallbackBuffer)]), "%c", PBAPEventData->Event_Data.PBAP_Pull_vCard_Listing_Confirmation_Data->Buffer[Index]);
            ...
            {
               Display(("Pull vCard Listing not complete, Requesting more data.\r\n"));
            ...
            else
            {
               ...
               Display(("Pull vCard Listing Completed.\r\n"));
            }
            break;
         case etPBAP_Pull_vCard_Entry_Confirmation:
            Display(("Pull vCard Entry Confirmation\r\n"));
            ...
            if(PBAPEventData->Event_Data.PBAP_Pull_vCard_Entry_Confirmation_Data->BufferSize)
            {
               ...
               for(Index = 0, TempCallbackBuffer[0] = '\0'; (Index<PBAPEventData->Event_Data.PBAP_Pull_vCard_Entry_Confirmation_Data->BufferSize) && (Index < (sizeof(TempCallbackBuffer) - 1)); Index++)
                  BTPS_SprintF(&(TempCallbackBuffer[BTPS_StringLength(TempCallbackBuffer)]), "%c", PBAPEventData->Event_Data.PBAP_Pull_vCard_Entry_Confirmation_Data->Buffer[Index]);
            ...
            {
               Display(("Pull vCard Entry not complete, Requesting more data.\r\n"));
            ...
            else
            {
               ...
               Display(("Pull vCard Entry Completed.\r\n"));
            }
            break;
         default:
            Display(("Unexpected (Unhandled) Event %d.\r\n", PBAPEventData->Event_Data_Type));
