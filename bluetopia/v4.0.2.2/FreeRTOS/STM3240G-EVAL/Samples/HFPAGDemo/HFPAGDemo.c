...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  HFPAGDemo - Simple embedded application using HFRE v1.6 Profile.          */
/*                                                                            */
/*  Author:   x0073106                                                        */
/*                                                                            */
...
#include "HFPAGDemo.h"       /* Application Header.                           */
...
#define APP_DEMO_NAME                          "HFPAGDemo"
...
static void BTPSAPI SDP_Event_Callback(unsigned int BluetoothStackID, unsigned int SDPRequestID, SDP_Response_Data_t *SDP_Response_Data, unsigned long CallbackParameter)
{
   ...
   DisplayPrompt();
	  ...
	  /* the indicators for the executing gateway server, since hopening   */
	  /* the remote hands free port is essentially opening a second AG.    */
	  /* Note that these indicators go nowhere, unless the command to open */
	  /* a handsfree server is executed.								   */
   static void InitializeDefaultIndicators(void)
   {
	  int Index = 0;
...
static int CloseClient(ParameterList_t *TempParam)
{
  ...
  return(ret_val);
   ...
   /* This function returns zero on successful execution and a negative */
   /* value on all errors.                                              */
   /*use as is Call Status, Call Setup, Service availability, Signal indicator, Roam indicator, battchg indicator, Call Held */
static int UpdateCurrentControlIndicatorStatus(ParameterList_t *TempParam)
{
   int                     ret_val;
   int                     Result;
   ...
   if(BluetoothStackID)
   {
      ...
      if(PortToUse)
      {
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            Display(("Usage: UPDATECONTROLINDICATORS [Name] [Value].\r\n"));
  ...
  /* zero on successful execution and a negative value on all errors.  */
  /* If the remote device callwaiting feature is supported in the */
  /* HF device than this will work */
...
static int SetRemoteVoiceRecognitionActivation(ParameterList_t *TempParam)
{
   ...
   /* execution and a negative value on all errors.                     */
   /* If the remote audio volume control feature is supported in the */
   /* HF device than this will work */
static int SetRemoteSpeakerGain(ParameterList_t *TempParam)
{
   int Result;
...
static int SetRemoteMicrophoneGain(ParameterList_t *TempParam)
{
   ...
   /* successful execution and a negative value on all errors.          */
   /* This function may be performed only when a valid Service Level */
...
static int DisableRemoteEchoCancelationNoiseReduction(ParameterList_t *TempParam)
{
   ...
   /* function returns zero on successful execution and a negative      */
   /* value on all errors.                                     */
static int SendOperatorSelection(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(BluetoothStackID)
   {
      ...
      if(CurrentClientPortID)
      {
         ...
         {
            /* We need an operatot name parameter.                      */
   ...
   /* the remote handsfree device. This function returns zero on successful */
   /* execution and a negative value on all errors.    */
static int SendSubscriberNumber(ParameterList_t *TempParam)
{
   int Result;
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters == 6))
      {
         ...
         {		
            ...
            /* Get call direction.                                      */
            CurrentCallListEntry.CallDirection = TempParam->Params[1].intParam;

            /* Get call status.                                         */
            CurrentCallListEntry.CallStatus = TempParam->Params[2].intParam;

            /* Get call Mode.                                           */
            CurrentCallListEntry.CallMode = TempParam->Params[3].intParam;
			...
			//Display(("CurrentCallListEntry.PhoneNumber  %s\r\n",CurrentCallListEntry.PhoneNumber));

            /* Finally, number format.                                  */
            CurrentCallListEntry.NumberFormat = HFRE_DEFAULT_NUMBER_FORMAT;
			//Display(("CurrentCallListEntry.NumberFormat  %d\r\n",CurrentCallListEntry.NumberFormat));
...
static int SendIncomingCallState(ParameterList_t *TempParam)
{
  ...
  if(BluetoothStackID)
  {
	 ...
	 {
		Display(("Usage: SendRespHold [CallState].\r\n"));
