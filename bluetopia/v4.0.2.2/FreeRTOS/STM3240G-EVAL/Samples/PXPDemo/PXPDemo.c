/*****< PXPDemo.c >************************************************************/
/*      Copyright 2012 - 2014 Stonestreet One, LLC.                           */
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*      Copyright 2015 Texas Instruments Incorporated.                        */
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  PXPDemo - Proximity Profile demonstration application.                    */
/*                                                                            */
/*** MODIFICATION HISTORY *****************************************************/
/*                                                                            */
/*   mm/dd/yy  F. Lastname    Description of Modification                     */
/*   --------  -----------    ------------------------------------------------*/
/*   05/28/12  R. Byrne       Initial creation.                               */
...
#include "PXPDemo.h"             /* Application Header.                       */
...
#define MAX_SUPPORTED_COMMANDS                      (43) /* Denotes the       */
                                                         /* maximum number of */
                                                         /* User Commands that*/
                                                         /* are supported by  */
                                                         /* this application. */
...
#define LE_APP_DEMO_NAME                        "PXPDemo"
...
static void ResolveRemoteAddressHelper(BD_ADDR_t BD_ADDR);
   ...
   /* Link Loss Service Profile Function Commands.                      */
static int RegisterLLS(ParameterList_t *TempParam);
static int UnregisterLLS(ParameterList_t *TempParam);
static int DiscoverLLS(ParameterList_t *TempParam);
static int GetAlertLevel(ParameterList_t *TempParam);
static int SetAlertLevel(ParameterList_t *TempParam);

   /* Link Loss Service Helper Functions                                */
static void LLSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
...
static void IASPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
   ...
   /* Tx Power Service Profile Function Commands                        */
static int RegisterTPS(ParameterList_t *TempParam);
static int UnregisterTPS(ParameterList_t *TempParam);
static int DiscoverTPS(ParameterList_t *TempParam);
static int GetTxPowerLevel(ParameterList_t *TempParam);
static int SetTxPowerLevel(ParameterList_t *TempParam);

   /* Tx Power Service Helper Functions                                 */
static void TPSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData);
...
static void BTPSAPI GATT_ClientEventCallback_TPS(unsigned int BluetoothStackID, GATT_Client_Event_Data_t *GATT_Client_Event_Data, unsigned long CallbackParameter);
...
static void UserInterface(void)
{
   ...
   AddCommand("REGISTERLLS", RegisterLLS);
   AddCommand("UNREGISTERLLS", UnregisterLLS);
   AddCommand("DISCOVERLLS", DiscoverLLS);
   AddCommand("GETALERTLEVEL", GetAlertLevel);
   AddCommand("SETALERTLEVEL", SetAlertLevel);
   ...
   AddCommand("REGISTERTPS",   RegisterTPS);
   AddCommand("UNREGISTERTPS", UnregisterTPS);
   AddCommand("DISCOVERTPS", DiscoverTPS);
   AddCommand("GETTXPOWERLEVEL",GetTxPowerLevel);
   AddCommand("SETTXPOWERLEVEL",SetTxPowerLevel);
   ...
   if((TempCommand) && (TempCommand->Command))
   {
      ...
      if(BTPS_MemCompare(TempCommand->Command, "QUIT", BTPS_StringLength("QUIT")) != 0)
      {
         ...
         if((CommandFunction = FindCommand(TempCommand->Command)) != NULL)
         {
            ...
            if((ret_val != 0) && (ret_val != EXIT_CODE) && (ret_val != EXIT_TEST_MODE))
...
static void GAPSPopulateHandles(GAPS_Client_Info_t *ClientInfo, GATT_Service_Discovery_Indication_Data_t *ServiceInfo)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a LLS Client Information structure with   */
...
static void LLSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((DeviceInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (LLS_COMPARE_LLS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service     */
      /* and populate the correct entry.                                */
      ...
      if(CurrentCharacteristic)
      {
         ...
         {
            /* All LLS UUIDs are defined to be 16 bit UUIDs.            */
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               /* Determine which characteristic this is.               */
               if(LLS_COMPARE_LLS_ALERT_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  DeviceInfo->LLS_ClientInfo.Alert_Level = CurrentCharacteristic->Characteristic_Handle;

                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     Display(("Warning - Mandatory read property of LLS Alert Level characteristic not supported!\r\n"));

                  /* Verify that Write is supported.                    */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_WRITE))
                     Display(("Warning - Mandatory write property of LLS Alert Level characteristic not supported!\r\n"));
               }
               else
               {
                  Display(("Unknown LLS characteristic detected!\r\n"));
               }
            }
         }
      }
...
static void IASPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   ...
   /* The following function is a utility function that provides a      */
   /* mechanism of populating a TPS Client Information structure with   */
   /* the information discovered from a GATT Discovery operation.       */
static void TPSPopulateHandles(DeviceInfo_t *DeviceInfo, GATT_Service_Discovery_Indication_Data_t *ServiceDiscoveryData)
{
   unsigned int                       Index;
   ...
   /* Verify that the input parameters are semi-valid.                  */
   if((DeviceInfo) && (ServiceDiscoveryData) && (ServiceDiscoveryData->ServiceInformation.UUID.UUID_Type == guUUID_16) && (TPS_COMPARE_TPS_SERVICE_UUID_TO_UUID_16(ServiceDiscoveryData->ServiceInformation.UUID.UUID.UUID_16)))
   {
      /* Loop through all characteristics discovered in the service     */
      /* and populate the correct entry.                                */
      CurrentCharacteristic = ServiceDiscoveryData->CharacteristicInformationList;
      if(CurrentCharacteristic)
      {
         for(Index = 0; Index < ServiceDiscoveryData->NumberOfCharacteristics; Index++, CurrentCharacteristic++)
         {
            /* All TPS UUIDs are defined to be 16 bit UUIDs.            */
            if(CurrentCharacteristic->Characteristic_UUID.UUID_Type == guUUID_16)
            {
               /* Determine which characteristic this is.               */
               if(TPS_COMPARE_TPS_TX_POWER_LEVEL_UUID_TO_UUID_16(CurrentCharacteristic->Characteristic_UUID.UUID.UUID_16))
               {
                  DeviceInfo->TPS_ClientInfo.Tx_Power_Level = CurrentCharacteristic->Characteristic_Handle;

                  /* Verify that read is supported.                     */
                  if(!(CurrentCharacteristic->Characteristic_Properties & GATT_CHARACTERISTIC_PROPERTIES_READ))
                     Display(("Warning - Mandatory read property of Tx Power Level characteristic not supported!\r\n"));

                  continue;
               }
               else
               {
                  Display(("Unknown TPS characteristic detected!\r\n"));
               }
            }
         }
      }
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   Display(("* Command Options LLS:     RegisterLLS, UnRegisterLLS,           *\r\n"));
   Display(("*                          DiscoverLLS,                          *\r\n"));
   Display(("*                          GetAlertLevel,                        *\r\n"));
   Display(("*                          SetAlertLevel,                        *\r\n"));
   Display(("* Command Options IAS:     RegisterIAS, UnRegisterIAS,           *\r\n"));
   Display(("*                          DiscoverIAS,                          *\r\n"));
   Display(("*                          ConfigureAlertLevel,                  *\r\n"));
   Display(("* Command Options TPS:     RegisterTPS, UnRegisterTPS,           *\r\n"));
   Display(("*                          DiscoverTPS,                          *\r\n"));
   Display(("*                          SetTxPowerLevel,                      *\r\n"));
   Display(("*                          GetTxPowerLevel,                      *\r\n"));
   Display(("******************************************************************\r\n"));
...
static int SetBaudRate(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters > 0) && (TempParam->Params[0].intParam))
      {
         ...
         if((!ret_val) && (!Status))
         {
            /* We were successful, now we need to change the baud rate  */
            /* of the driver.                                           */
            ...
            if(ret_val >= 0)
            {
               Display(("HCI_Reconfigure_Driver(%u): Success.\r\n", (TempParam->Params[0].intParam)));
            ...
            {
               Display(("HCI_Reconfigure_Driver(%u): Failure %d.\r\n", (TempParam->Params[0].intParam), ret_val));
         ...
         else
         {
            ...
            Display(("HCI_Send_Raw_Command(%u): Failure %d, %d.\r\n", (TempParam->Params[0].intParam), ret_val, Status));
         ...
         if  ((TempParam->NumberofParameters >= 1) && ((TempParam->Params[0].intParam == 0) || (TempParam->Params[0].intParam == 1)))
         {
             ...
             else
             {
                ...
                if (!ret_val)
                {
                   ...
                   if(LLSInstanceID)
                   {
                      /* Advertise the LLS Server                           */
                      /* (1 byte type & 2 bytes UUID).                      */
                      ...
                      LLS_ASSIGN_LLS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[5]));
                      AdvertiseDataLength                                           = 2;
                   }
                   if(TPSInstanceID)
                   {
                      /* Advertise the TPS Server                           */
                      /* (1 byte type & 2 bytes UUID).                      */
                      Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[7] = 3;
                      Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[8] = HCI_LE_ADVERTISING_REPORT_DATA_TYPE_16_BIT_SERVICE_UUID_COMPLETE;

                      TPS_ASSIGN_TPS_SERVICE_UUID_16(&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[9]));
                      AdvertiseDataLength                                          += 2;
                   }
                   if(IASInstanceID)
                   {
                      /* Advertise the IAS Server                           */
                      /* (1 byte type & 2 bytes UUID).                      */
   ...
   if(TempParam->NumberofParameters >= 1)
   {
      if(ConnectionID)
      {
         ...
         if((RequestedMTU > ATT_PROTOCOL_MTU_MINIMUM_LE) && (RequestedMTU <= ATT_PROTOCOL_MTU_MAXIMUM))
         {
            ...
            if(ret_val >= 0)
            {
               Display(("GATT_Exchange_MTU_Request Sucess.\r\n"));
               ret_val = 0;
            }
            else
            {
...
static int RegisterLLS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!LLSInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            Display(("Successfully registered LLS Service.\r\n"));
   ...
   /* The following function is responsible for unregistering a LLS     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterLLS(ParameterList_t *TempParam)
{
   ...
   if(LLSInstanceID)
   {
      ...
      /* Unregister the LLS Service with GATT.                          */
      ret_val = LLS_Cleanup_Service(BluetoothStackID, LLSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         Display(("Successfully unregistered LLS Service.\r\n"));

         /* Save the ServiceID of the registered service.               */
         LLSInstanceID = 0;
      }
      else
         DisplayFunctionError("LLS_Cleanup_Service", ret_val);
   }
   else
      Display(("LLS Service not registered.\r\n"));

   return(ret_val);
}

   /* The following function is responsible for performing a LLS        */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverLLS(ParameterList_t *TempParam)
{
   ...
   if(!LLSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               /* Configure the filter so that only the LLS Service is  */
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               LLS_ASSIGN_LLS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdLLS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  Display(("GATT_Start_Service_Discovery success.\r\n"));
      ...
      Display(("Cannot discover LLS Services when registered as a service.\r\n"));

   return(ret_val);
}

   /* The following function is responsible for reading the Alert       */
   /* Level. It can be executed by a server or as a client with         */
...
static int GetAlertLevel(ParameterList_t *TempParam)
{
   ...
   /* First check for a registered LLS Server                           */
   if(LLSInstanceID)
   {
      ret_val = LLS_Query_Alert_Level(BluetoothStackID, LLSInstanceID, &AlertLevel);
      if(ret_val == 0)
      {
         Display(("LLS Alert Level: %u", AlertLevel));
      }
      else
         DisplayFunctionError("LLS_Query_Alert_Level", ret_val);
   }
   else
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Verify that the client has received a valid LLS          */
            /* Attribute Handle.                                        */
            if(DeviceInfo->LLS_ClientInfo.Alert_Level != 0)
            {
               /* Finally, submit a read request to the server.         */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->LLS_ClientInfo.Alert_Level, GATT_ClientEventCallback_LLS, DeviceInfo->LLS_ClientInfo.Alert_Level)) > 0)
               {
                  Display(("Get Alert Level request sent, Transaction ID = %u", ret_val));
               ...
               Display(("Error - LLS Alert Level attribute handle is invalid!\r\n"));
         }
         else
            Display(("No Device Info.\r\n"));
      }
      else
         Display(("Either a LLS server must be registered or a LLS client must be connected.\r\n"));
   }
   ...
   /* connection to a remote server if client write access is supported.*/
   /* If executed as a client, a GATT write request will be generated,  */
   /* and the results will be returned as a response in the             */
   /* GATT client event callback.  This function will return zero on    */
   /* successful execution and a negative value on errors.              */
static int SetAlertLevel(ParameterList_t *TempParam)
{
   Byte_t            AlertLevel;
   NonAlignedByte_t  Data;
   DeviceInfo_t     *DeviceInfo;
   int               ret_val = FUNCTION_ERROR;

   if((TempParam) && (TempParam->NumberofParameters > 0))
   {
      /* Get AlertLevel as Input                                        */
      AlertLevel = (Byte_t)TempParam->Params[0].intParam;
      if(LLS_ALERT_LEVEL_VALID(AlertLevel))
      {
         if(LLSInstanceID)
         {
            if((ret_val = LLS_Set_Alert_Level(BluetoothStackID, LLSInstanceID, AlertLevel)) == 0)
            {
               Display(("Alert Level successfully set to %u. \r\n", AlertLevel));
            }
            else
               DisplayFunctionError("LLS_Set_Alert_Level", ret_val);
         }
         else
         {
            if(ConnectionID)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  ...
                  {
                     /* Format the data                                    */
                     ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&Data, AlertLevel)

                     /* Finally, submit a write request to the server      */
                     if((ret_val = GATT_Write_Request(BluetoothStackID, ConnectionID, DeviceInfo->LLS_ClientInfo.Alert_Level, sizeof(NonAlignedByte_t), (void *)&Data, GATT_ClientEventCallback_LLS, DeviceInfo->LLS_ClientInfo.Alert_Level)) > 0)
                     {
                        Display(("Set Alert Level Request sent, Transaction ID = %u", ret_val));
                     ...
                     Display(("Error - Alert Level attribute handle is invalid!\r\n"));
      ...
      else
      {
         ...
         DisplayUsage("SetAlertLevel [AlertLevel]");
         Display(("   Where AlertLevel = \r\n"));
         Display(("      0 : No Alert\r\n"));
         Display(("      1 : Mild Alert\r\n"));
         Display(("      2 : High Alert\r\n"));
      }
   }
   else
   {
...
static int ConfigureAlertLevel(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1))
   {
      ...
      if(IAS_ALERT_LEVEL_VALID(Input))
      {
         /* Verify that we are not configured as a Server               */
         if(!IASInstanceID)
         {  /* Verify that there is a valid connection                  */
            if(ConnectionID)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
               {
                  /* Verify that the client has recieved a valid Alert  */
                  /* Level Control Point.                               */
...
static int RegisterTPS(ParameterList_t *TempParam)
{
   ...
   if(!ConnectionID)
   {
      ...
      if(!TPSInstanceID)
      {
         ...
         {
            /* Display success message.                                 */
            Display(("Successfully registered TPS Service.\r\n"));
   ...
   /* The following function is responsible for unregistering a TPS     */
   /* Service.  This function will return zero on successful execution  */
   /* and a negative value on errors.                                   */
static int UnregisterTPS(ParameterList_t *TempParam)
{
   ...
   if(TPSInstanceID)
   {
      ...
      /* Unregister the TPS Service with GATT.                          */
      ret_val = TPS_Cleanup_Service(BluetoothStackID, TPSInstanceID);
      if(ret_val == 0)
      {
         /* Display success message.                                    */
         Display(("Successfully unregistered TPS Service.\r\n"));

         /* Save the ServiceID of the registered service.               */
         TPSInstanceID = 0;
      }
      else
         DisplayFunctionError("TPS_Cleanup_Service", ret_val);
   }
   else
      Display(("TPS Service not registered.\r\n"));

   return(ret_val);
}

   /* The following function is responsible for performing a TPS        */
   /* Service Discovery Operation.  This function will return zero on   */
   /* successful execution and a negative value on errors.              */
static int DiscoverTPS(ParameterList_t *TempParam)
{
   ...
   if(!TPSInstanceID)
   {
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            ...
            if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
            {
               ...
               /* discovered.                                           */
               UUID[0].UUID_Type = guUUID_16;
               TPS_ASSIGN_TPS_SERVICE_UUID_16(&(UUID[0].UUID.UUID_16));

               /* Start the service discovery process.                  */
               ret_val = GATT_Start_Service_Discovery(BluetoothStackID, ConnectionID, (sizeof(UUID)/sizeof(GATT_UUID_t)), UUID, GATT_Service_Discovery_Event_Callback, sdTPS);
               if(!ret_val)
               {
                  /* Display success message.                           */
                  Display(("GATT_Start_Service_Discovery success.\r\n"));
      ...
      Display(("Cannot discover TPS Services when registered as a service.\r\n"));

   return(ret_val);
}

   /* The following function is responsible for reading the Tx Power    */
   /* Level. It can be executed by a server or as a client with         */
   /* an open connection to a remote server.  If executed as a client,  */
   /* a GATT read request will be generated, and the results will be    */
   /* returned as a response in the GATT client event callback.  This   */
...
static int GetTxPowerLevel(ParameterList_t *TempParam)
{
   ...
   /* First check for a registered TPS Server                           */
   if(TPSInstanceID)
   {
      ret_val = TPS_Query_Tx_Power_Level(BluetoothStackID, TPSInstanceID, &TxPowerLevel);
      if(ret_val == 0)
      {
         Display(("Tx Power Level: %d", TxPowerLevel));
      }
      else
         DisplayFunctionError("TPS_Query_Tx_Power_Level", ret_val);
   }
   else
   {
      /* Check to see if we are configured as a client with an active   */
      ...
      if(ConnectionID)
      {
         ...
         if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
         {
            /* Verify that the client has received a valid TPS          */
            /* Attribute Handle.                                        */
            if(DeviceInfo->TPS_ClientInfo.Tx_Power_Level != 0)
            {
               /* Finally, submit a read request to the server.         */
               if((ret_val = GATT_Read_Value_Request(BluetoothStackID, ConnectionID, DeviceInfo->TPS_ClientInfo.Tx_Power_Level, GATT_ClientEventCallback_TPS, DeviceInfo->TPS_ClientInfo.Tx_Power_Level)) > 0)
               {
                  Display(("Get Tx Power Level request sent, Transaction ID = %d\r\n", ret_val));
               ...
               Display(("Error - Tx Power Level attribute handle is invalid!\r\n"));
         }
         else
            Display(("No Device Info.\r\n"));
      }
      else
         Display(("Either a TPS server must be registered or a TPS client must be connected.\r\n"));
   }

   return(ret_val);
}

   /* The following function is responsible for writing the Tx Power    */
   /* Level. It can be executed only by a server. This function         */
...
static int SetTxPowerLevel(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters >= 1))
   {
      /* Check for a registered TPS Server.                             */
      if(TPSInstanceID)
      {
         ret_val = TPS_Set_Tx_Power_Level(BluetoothStackID, TPSInstanceID, (SByte_t)TempParam->Params[0].intParam);
         if(ret_val == 0)
         {
            Display(("Tx Power Level successfully set.\r\n"));
         }
         else
         {
            DisplayFunctionError("TPS_Set_Tx_Power_Level\r\n ", ret_val);
         }
      }
      else
      {
         if(ConnectionID)
         {
            Display(("Cannot write Tx Power Level as a client.\r\n"));
         }
         else
         {
            Display(("TPS server not registered.\r\n"));
         }
      }
   }
   else
   {
      DisplayUsage("SetTxPowerLevel [TxPowerLevel (Valid value range: (-100) - (20))]");
   }
      ...
      switch(LLS_Event_Data->Event_Data_Type)
      {
         case etLLS_Alert_Level_Update:
            ...
            if(LLS_Event_Data->Event_Data.LLS_Alert_Level_Update_Data)
            {
               ...
               if(SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, LLS_Event_Data->Event_Data.LLS_Alert_Level_Update_Data->RemoteDevice) != NULL)
               {
      ...
      switch(GATT_Client_Event_Data->Event_Data_Type)
      {
         case etGATT_Client_Error_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorCode == 0x81)
               {
                  Display(("   Error Mesg:    RACP Improperly Configured\r\n"));
               }
               else
               {
         ...
         case etGATT_Client_Read_Response:
            if(GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data)
            {
               ...
               if(ValueLength != 0)
               {
                  if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
                  {
                     if(CallbackParameter == DeviceInfo->TPS_ClientInfo.Tx_Power_Level)
                     {
                        Display(("\r\n   Tx Power Level: %d.\r\n", (SByte_t)GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->AttributeValue[0]));
                     }
                  }
...
static void BTPSAPI GATT_Service_Discovery_Event_Callback(unsigned int BluetoothStackID, GATT_Service_Discovery_Event_Data_t *GATT_Service_Discovery_Event_Data, unsigned long CallbackParameter)
{
         ...
         switch(GATT_Service_Discovery_Event_Data->Event_Data_Type)
         {
            case etGATT_Service_Discovery_Indication:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data)
               {
                  ...
                  /* Check the service discovery type for LLS           */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdLLS)
                  {
                     /* Attempt to populate the handles for the LLS     */
                     /* Service.                                        */
                     LLSPopulateHandles(DeviceInfo, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                  }
                  ...
                  /* Check the service discovery type for TPS           */
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdTPS)
                  {
                     /* Attempt to populate the handles for the TPS     */
                     /* Service.                                        */
                     TPSPopulateHandles(DeviceInfo, GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Indication_Data);
                  }
               }
               break;
            case etGATT_Service_Discovery_Complete:
               ...
               if(GATT_Service_Discovery_Event_Data->Event_Data.GATT_Service_Discovery_Complete_Data)
               {
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdLLS)
                  {
                     /* Flaf that service discovery has been performed  */
                     /* on for this connection.                         */
                     DeviceInfo->Flags |= DEVICE_INFO_FLAGS_LLS_SERVICE_DISCOVERY_COMPLETE;

                     /* Print a summary of what descriptor were found   */
                     Display(("\r\nLLS Service Discovery Summary\r\n"));
                     Display(("     Alert Level: %s\r\n", (DeviceInfo->LLS_ClientInfo.Alert_Level ? "Supported" : "Not Supported")));
                  }
                  ...
                  if(((Service_Discovery_Type_t)CallbackParameter) == sdTPS)
                  {
                     /* Flag that service discovery has been performed  */
                     /* on for this connection.                         */
                     DeviceInfo->Flags |= DEVICE_INFO_FLAGS_TPS_SERVICE_DISCOVERY_COMPLETE;

                     /* Print a summary of what descriptor were found   */
                     Display(("\r\nTPS Service Discovery Summary\r\n"));
                     Display(("    Tx_Power_Level: %s\r\n", (DeviceInfo->TPS_ClientInfo.Tx_Power_Level ? "Supported" : "Not Supported")));
                  }
               }
               break;
         }
   ...
   /* when opening the stack.  This function returns the                */
