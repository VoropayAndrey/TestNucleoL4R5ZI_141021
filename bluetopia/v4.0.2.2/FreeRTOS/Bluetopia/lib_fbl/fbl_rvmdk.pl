...
# File:        fbl_rvmdk.pl
# Author:      Tim Cook
# Description: Builds Flexible Build Library for RVMDK
# Changed on 14 june 2015- Add the processor build option for the library creation.
# Usage:       fbl_rvmdk.pl [--sppserver] [--sppclient] [--sdpserver] [--sdpclient]
...
my $SCO;
my @Objects;
my $BluetopiaLibrary = "Bluetopia_$ObjectsNameSuffix.lib";
my @GATTObjects;
my $GATTLibrary = "SS1BTGAT_$ObjectsNameSuffix.lib";
my @MAPObjects;
my $MAPLibrary = "SS1BTMAP_$ObjectsNameSuffix.lib";
my @PBAPObjects;
my $PBAPLibrary = "SS1BTPBA_$ObjectsNameSuffix.lib";
my @HIDObjects;
my $HIDLibrary  = "SS1BTHID_$ObjectsNameSuffix.lib";
my $BuildOutput;
my $Help;
my $Folder;
sub BuildLibraries;
...
print "\n";
print "############################################################################\n";
print "Running Flexible Build Library Utility.  RVMDK Archiver (armar) must be in the \npath variable.\n";
print "############################################################################\n";
print "\n";
...
{
print <<HELP;
   fbl_rvmdk.pl options (Note all options are optional):
...
# doron debug, WAS:   push(@Objects, "Objects\\$Directory\\HCID_$ObjectsNameSuffix.o");
   ...
   $BuildOutput = `armar --create $BluetopiaLibrary @Objects`;
   print $BuildOutput;
   ...
   mkdir($Folder, 0700) unless (-d $Folder);
   $Folder = "generated_libs/lib/rvmdk";
   mkdir($Folder, 0700) unless (-d $Folder);
   $Folder = "generated_libs/lib/rvmdk/$Directory";
   mkdir($Folder, 0700) unless (-d $Folder);
   ...
   {
      unlink($GATTLibrary);
      $BuildOutput = `armar --create $GATTLibrary @GATTObjects`;
      ...
      mkdir($Folder, 0700) unless (-d $Folder);
      $Folder = "generated_libs/profiles/gatt/lib/rvmdk";
      mkdir($Folder, 0700) unless (-d $Folder);
   ...
   {
      unlink($HIDLibrary);
      $BuildOutput = `armar --create $HIDLibrary @HIDObjects`;
      ...
      mkdir($Folder, 0700) unless (-d $Folder);
      $Folder = "generated_libs/profiles/hid/lib/rvmdk";
      mkdir($Folder, 0700) unless (-d $Folder);
   ...
   # {
      # unlink($MAPLibrary);
      # $BuildOutput = `armar --create $MAPLibrary @MAPObjects`;
      ...
      # mkdir($Folder, 0700) unless (-d $Folder);
      # $Folder = "generated_libs/profiles/map/lib/rvmdk";
      # mkdir($Folder, 0700) unless (-d $Folder);
   ...
   # {
      # unlink($PBAPLibrary);
      # $BuildOutput = `armar --create $PBAPLibrary @PBAPObjects`;
      ...
      # mkdir($Folder, 0700) unless (-d $Folder);
      # $Folder = "generated_libs/profiles/pbap/lib/rvmdk";
      # mkdir($Folder, 0700) unless (-d $Folder);
