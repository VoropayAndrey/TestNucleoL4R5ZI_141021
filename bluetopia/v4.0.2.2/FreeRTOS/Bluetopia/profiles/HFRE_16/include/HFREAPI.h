...
BTPSAPI_DECLARATION int BTPSAPI HFRE_Send_Available_Codecs(unsigned int BluetoothStackID, unsigned int HFREPortID, unsigned int NumberAvailableCodecs, unsigned char *CodecIDList);
   ...
   /* may only be performed by an Audio Gateway with a valid Service    */
   /* Level Connection.  This function accepts as its input parameters  */
   /* the Bluetooth Stack ID for which the HFRE Port ID is valid as well*/
   /* as the HFRE Port ID.  The final parameter to this function is the */
   /* Codec ID of the Codec that is being selected.  This function      */
